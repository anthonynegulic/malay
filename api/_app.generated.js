...
function tokenise(rawText, multiWordUnits) {
  ...
  return text.split(/[^a-zà-ɏ'\-_0-9]+/i).map((t) => t.replace(/^[-']+|[-']+$/g, "")).filter(Boolean);
}
function isNumeral(token) {
  return /^[0-9]+$/.test(token);
}
function properNouns(rawText) {
...
function config() {
  return {
    ...
    // optional; when set, gates /generate + /grade
  };
}
var GENERATE_SYSTEM = `You are a Malay-language content generator for a beginner preparing to live in
...
function normaliseGenOut(raw2) {
  ...
  return {
    ...
    translation: typeof raw2.translation === "string" ? raw2.translation : "",
    glossary: Array.isArray(raw2.glossary) ? raw2.glossary.filter(
...
app.post("/generate", async (c) => {
  ...
  try {
    const first = normaliseGenOut(
      await callAndParse(GENERATE_SYSTEM, JSON.stringify(payload))
    );
    let chosen = first;
